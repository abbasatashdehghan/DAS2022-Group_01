...
Recast variables
add a new column called "Number_of_Family_new", reducing the level of "Number_of_Family" from 15 to 2 for binomial 
```{r}
data$Region <- factor(data$Region)
data$Gender <- factor(data$Gender)
data$Type <- factor(data$Type)    
data$Number_of_Family_new <- factor(ifelse(data$Number_of_Family >= 7,1,0))
summary(data)
```

first time check, confirm outliers
```{r}
data_check <- data %>% summarize( Number_of_Family_new = (Number_of_Family_new),
                                  Income = (Income),
...
model <- glm(Number_of_Family_new ~ Income+FoodExpenditure+Gender+Age+Type+Area+HouseAge+bedrooms+Electricity, 
             data = data_check, family = 'binomial')
summary(model)
...
summary(continuous)
```
data have totally different scales and many of them have large outliers, try standardize them

Income
98% is 1005519, drop the observations above this threshold
```{r}
...
FoodExpenditure
99% is 226730.3, drop the observations above this threshold
```{r}
...
Age
Since the 99% is 85, remain all data as it looks fine
```{r}
...
Area
99% is 500, drop the observations above this threshold
```{r}
...
bedrooms 
99% is 6, drop the observations above this threshold
```{r}
...
```{r}
model <- glm(Number_of_Family_new ~       FoodExpenditure+Gender+Age+Type+Area+HouseAge+bedrooms+Electricity, 
