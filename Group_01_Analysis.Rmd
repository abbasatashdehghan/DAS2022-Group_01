...
```
The full model data in terms of AIC, BIC and can be seen Table \ref{tabel:fullmodel}

```{r fullmodel}
Models = c("Poisson")
model.poisson.params <- round(glance(model.poisson),1)
McFadden <- data.frame(pseudo_R2 = c(PseudoR2(model.poisson)))

bind_rows(model.poisson.params, .id = "Model") %>% 
  select(Model, AIC, BIC) %>% 
  mutate(Model = Models) %>% cbind(McFadden) %>% remove_rownames(.) %>% 
  kable(caption = "\\label{tabel:fullmodel} Full model data",align = rep("c", 4), digits = 3)
```
...
Table \ref{tabel:finalmodel} presents AIC, BIC and Pseudo-R2 for the final model:
...
bind_rows(model.poisson.params, .id = "Model") %>% 
  select(Model, AIC, BIC) %>% 
  mutate(Model = Models) %>% cbind(McFadden) %>% remove_rownames(.) %>% 
  kable(caption = "\\label{tabel:finalmodel} Final model data",align = rep("c", 4), digits = 3)
```
...
grid.arrange(Log.Income, Log.Expenditure, Gender, Type, Bedrooms)
```
