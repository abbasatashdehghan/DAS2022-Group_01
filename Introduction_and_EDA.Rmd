...
data$Type[data$Type == "Two or More Nonrelated Persons/Members"] <- "Other"
...
## Recast variables
From the above it can been seen that variable "Region" will have no contribution to the upcoming analysis as it has only one state. Therefore, it is removed from the data. 
...
# summary(data)
data[,-c(3,5,11)] %>% skim() %>% 
  select(skim_variable,numeric.p0, numeric.p25,
                       numeric.mean, numeric.sd, numeric.p50, numeric.p75, numeric.p100) %>% 
...
Referring to \ref{tabel:summary}, the difference between mean and median of some variables also indicating the presence of large outliers in the data. The following pair plots fuhrer support the presence of outliers.
...
Figure \ref{fig:outliers} presents the distribution of outcome variables with regards to each of the continuous variables. The boxplots can further help identifying the outliers of each variable.  
...
In this section we model the data to identify the most influential factors on the number of family members using different GLMs. Then the goodness of fit comparison is made to select the best model based on AIC and Deviance.
...
# Model Comparison
We now have 4 models. The Table \ref{tabel:models} shows the models AIC, BIC and their associated McFadden’s pseudo-$R^2$ value.
```{r modelcomparison}
